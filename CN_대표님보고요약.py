...
- 모든 금액은 천 단위(k)로 표시 (원본 데이터를 1,000으로 나누어 표기)
- 단위는 k, 3자리마다 쉼표 표기
- ⚠️ **중요: 천 단위 표시 시 반드시 정수로 표기하고 소수점을 사용하지 말 것**
  - 올바른 예: 1,234k, 588k, 1,378k
  - 잘못된 예: 1.2k, 0.5k, 1,234.5k
...
def extract_key_from_filename(filename):
    ...
    # CN_2512_I_출고매출_카테고리별매출분석
    # parts[0] = CN
    # parts[1] = 2512
    # parts[2] = I
    # parts[3:] = ['출고매출', '카테고리별매출분석']
    
    country = parts[0] if parts[0] in ['CN', 'KR'] else 'CN'
...
def load_json_files_for_month_brand(yyyymm, brd_cd):
    ...
    pattern = os.path.join(OUTPUT_JSON_PATH, f"CN_{yyyymm_short}_{brd_cd}_*.json")
    ...
    if not json_files:
        ...
        # 전체 파일 목록 확인 (디버깅용)
        all_files = glob.glob(os.path.join(OUTPUT_JSON_PATH, f"CN_{yyyymm_short}_*.json"))
        if all_files:
...
{{
  ...
  "sections": [
    {{
      "div": "종합분석-1",
      "sub_title": "전체 성과 요약",
      "ai_text": "해당 월의 전체 성과를 구체적으로 요약. 주요 매출액(천 단위), 전년대비 성장률, 주요 지표 변화 등을 구체적인 수치와 함께 제시. (3-4줄)"
    }},
    {{
      "div": "종합분석-2",
      "sub_title": "주요 인사이트",
...
- 모든 수치는 천 단위(k)로 표시하고 구체적인 수치를 반드시 포함
- 실행 가능한 전략을 제시
...
"""
    ...
    filename = f"CN_{yyyymm_short}_{brd_cd}_월별브랜드통합요약"
...
def generate_monthly_all_brands_summary(yyyymm):
    ...
    yyyymm_short = yyyymm[2:]
    pattern = os.path.join(OUTPUT_JSON_PATH, f"CN_{yyyymm_short}_*_월별브랜드통합요약.json")
...
{{
  ...
  "sections": [
    {{
      "div": "종합분석-1",
      "sub_title": "전체 성과 요약",
      "ai_text": "모든 브랜드를 통합한 전체 성과를 구체적으로 요약. 전체 매출액(천 단위), 전년대비 성장률, 브랜드별 기여도, 주요 지표 변화 등을 구체적인 수치와 함께 제시. (3-4줄)"
    }},
    {{
      "div": "종합분석-2",
...
"""
    ...
    # 파일 저장
    yyyymm_short = yyyymm[2:]
    filename = f"CN_{yyyymm_short}_ALL_월별전체브랜드통합요약"
    save_json(json_data, filename)
...
if __name__ == '__main__':
    ...
    print(f"\n{'='*60}")
    print(f"CN 대표님 보고용 통합 요약 생성 시작")
    ...
    # 방법 1: 한 달만 분석 (예: 2025년 12월만)
    yyyymm_list = generate_yyyymm_list('202512')
    
    # 방법 2: 여러 달 분석 (시작월 ~ 종료월)
    # 예: 2025년 1월부터 12월까지
    # yyyymm_list = generate_yyyymm_list('202501', '202512')
    
    # 방법 3: 직접 리스트 지정 (원하는 월만 선택)
    # 예: 2025년 9월, 10월, 11월, 12월만
    # yyyymm_list = ['202509', '202510', '202511', '202512']
    ...
    for yyyymm in yyyymm_list:
        ...
        if brand_summary_success_count > 0:
            ...
            print(f"[INFO] 먼저 cn_analysis.py를 실행하여 해당 월의 분석 결과를 생성해주세요.\n")
