        ...
        try:
            for chnl_nm in channel_summary.keys():
                try:
                    ...
                    for record in records:
                        try:
                            # PST_YYYYMM을 문자열로 변환하여 비교
                            record_month = str(record.get('pst_yyyymm', ''))
                            if (record_month == yyyymm and 
                                record.get('chnl_nm') == chnl_nm):
                                class3 = str(record.get('class3', '') or '기타').strip()
                                if not class3:
                                    class3 = '기타'
                                sale_amt_val = record.get('sale_amt', 0)
                                if sale_amt_val is None:
                                    sale_amt_val = 0
                                sale_amt = float(sale_amt_val)
                                if class3 not in curr_month_items:
                                    curr_month_items[class3] = 0
                                curr_month_items[class3] += sale_amt
                        except Exception as e:
                            print(f"[WARNING] 당해 데이터 처리 중 오류 (채널: {chnl_nm}): {e}")
                            continue
                        ...
                        try:
                            ...
                            if (record_month == yyyymm_py and 
                                ...
                                if sale_amt_val is None:
                                    sale_amt_val = 0
                                sale_amt = float(sale_amt_val)
                                if class3 not in prev_month_items:
                                    prev_month_items[class3] = 0
                                prev_month_items[class3] += sale_amt
                        except Exception as e:
                            print(f"[WARNING] 전년 데이터 처리 중 오류 (채널: {chnl_nm}): {e}")
                            continue
                    
                    # 당해 TOP 3 정렬
                    curr_top3 = sorted(
                        [(k, v) for k, v in curr_month_items.items() if v > 0],
                        key=lambda x: x[1],
                        reverse=True
                    )[:3]
                    ...
                    # 전년 TOP 3 정렬
                    prev_top3 = sorted(
                        [(k, v) for k, v in prev_month_items.items() if v > 0],
                        key=lambda x: x[1],
                        reverse=True
                    )[:3]
                    
                    channel_best_items[chnl_nm] = {
                        'current_year': {
                            'yyyymm': yyyymm,
                            'top3': [
                                {
                                    'class3': item[0],
                                    'sale_amt': round(item[1] / 1000000, 2)
                                }
                                for item in curr_top3
                            ] if curr_top3 else []
                        },
                        'previous_year': {
                            'yyyymm': yyyymm_py,
                            'top3': [
                                ...
                                for item in prev_top3
                            ] if prev_top3 else []
                        }
                    }
                except Exception as e:
                    print(f"[WARNING] 채널 {chnl_nm} 베스트 아이템 추출 중 오류: {e}")
                    channel_best_items[chnl_nm] = {
                        'current_year': {'yyyymm': yyyymm, 'top3': []},
                        'previous_year': {'yyyymm': yyyymm_py, 'top3': []}
                    }
                    continue
        except Exception as e:
            print(f"[ERROR] 채널별 베스트 아이템 추출 중 전체 오류: {e}")
            import traceback
            traceback.print_exc()
        
        # 채널별 베스트 아이템 섹션 동적 생성
        channel_sections = []
        for chnl_nm in sorted(channel_summary.keys()):
            try:
                best_items = channel_best_items.get(chnl_nm, {})
                curr_top3 = best_items.get('current_year', {}).get('top3', [])
                prev_top3 = best_items.get('previous_year', {}).get('top3', [])
                
                # 채널별 데이터를 JSON으로 변환
                channel_data = {
                    'channel_name': chnl_nm,
                    'current_year_top3': curr_top3 if curr_top3 else [],
                    'previous_year_top3': prev_top3 if prev_top3 else [],
                    'current_year_total': channel_summary.get(chnl_nm, {}).get('total_sales', 0)
                }
                ...
                print(f"[WARNING] 채널 {chnl_nm} 데이터 처리 중 오류: {e}")
                continue
...
<채널별 베스트 아이템 데이터>
{json.dumps(channel_sections, ensure_ascii=False, indent=2)}

<요구사항>
아래 JSON 형식으로 분석 결과를 반환해줘. 반드시 유효한 JSON 형식이어야 하고, 마크다운 코드 블록 없이 순수 JSON만 반환해줘.

위의 <채널별 베스트 아이템 데이터>를 참고하여, 각 채널별로 "{{채널명}} 베스트 아이템 전년 대비 주요 변화" 섹션을 동적으로 생성해줘.
각 채널의 당해 베스트 아이템 TOP 3와 전년 동월 베스트 아이템 TOP 3를 비교하여 주요 변화를 3줄로 설명해줘.
...
<작성 가이드라인>
- 각 채널별로 "{{채널명}} 베스트 아이템 전년 대비 주요 변화" 섹션을 생성 (예: "면세점 베스트 아이템 전년 대비 주요 변화")
- 각 채널의 당해 베스트 아이템 TOP 3와 전년 동월 베스트 아이템 TOP 3를 비교하여 주요 변화를 3줄로 설명
- 각 섹션의 ai_text는 구체적이고 실용적인 내용으로 작성
- 숫자는 백만원 단위로 표시하고 절대 변형하지 말 것
- 모든 채널과 주요 아이템 누락 없이 분석
- 채널별 성장 패턴과 변화 추이를 구체적으로 분석
...
- 채널별 베스트 아이템 섹션은 채널별 성과 종합 평가 섹션 앞에 배치
...
"""
        ...
        if json_start >= 0 and json_end > json_start:
            analysis_response = analysis_response[json_start:json_end]
        
        try:
            analysis_data = json.loads(analysis_response)
        except json.JSONDecodeError as e:
            print(f"[WARNING] JSON 파싱 실패: {e}")
            print(f"[WARNING] 응답 내용 (처음 500자): {analysis_response[:500]}")
            print(f"[WARNING] 응답 내용 (마지막 500자): {analysis_response[-500:]}")
            # 기본 구조로 대체
            analysis_data = {
                "title": "채널별 매출 분석 (12개월 추이)",
                "sections": [
                    {"sub_title": "분석 결과", "ai_text": analysis_response[:2000] if len(analysis_response) > 2000 else analysis_response}
                ]
            }
        ...
        json_data = {
            ...
            'channel_best_items': channel_best_items,
            'raw_data': {
                'sample_records': [
                    {
        ...
        # 파일 저장
        filename = f"4-1-1-1.{brd_cd}_채널별_매출분석(12개월추이)"
...
if __name__ == '__main__':
    ...
    # 분석 실행 (원하는 분석만 주석 해제)
    analyze_channel_sales(yyyymm, brd_cd)  # 채널별 매출 분석
    # analyze_ad_expense(yyyymm, brd_cd)  # 광고선전비 추이 분석
